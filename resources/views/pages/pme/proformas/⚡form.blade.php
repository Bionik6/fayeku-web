...
class extends Component {
    ...
    public function mount(?ProposalDocument $proforma = null): void
    {
        ...
        if ($proforma && $proforma->exists) {
            ...
            $this->proforma = $proforma->load('lines', 'client');
            $this->isEditing = true;
            $this->reference = $proforma->reference ?? '';
            $this->clientId = $proforma->client_id ?? '';
            $this->issuedAt = $proforma->issued_at?->format('Y-m-d') ?? '';
            $this->validUntil = $proforma->valid_until?->format('Y-m-d') ?? '';
            $this->currency = $proforma->currency ?? 'XOF';
            $this->discount = $proforma->discount ?? 0;
            $this->discountType = $proforma->discount_type ?? 'percent';
            $this->hasDiscount = ($this->discount ?? 0) > 0;
            $this->notes = $proforma->notes ?? '';
            $this->hasNotes = $this->notes !== '';
            $this->dossierReference = $proforma->dossier_reference ?? '';
            $this->paymentTerms = $proforma->payment_terms ?? '';
            $this->deliveryTerms = $proforma->delivery_terms ?? '';
            $this->validityPreset = 'custom';

            $firstLine = $proforma->lines->first();
    ...
    public function hasFormData(): bool
    {
        ...
        if ($this->dossierReference !== '' || $this->paymentTerms !== '' || $this->deliveryTerms !== '') {
            return true;
        }
    ...
    public function confirmSaveDraft(): void
    {
        ...
        $this->redirect(route('pme.proformas.index').'?statut=draft', navigate: true);
    }
    ...
    public function saveDraft(bool $notify = true): void
    {
        ...
        if ($this->isEditing) {
            $service->update($this->proforma, $data, $lines);
    ...
    /**
     * Save the proforma as Draft and redirect to its show page with `?send=1`,
    ...
    public function saveAndSend(): void
    {
        ...
        if ($totals['total'] <= 0) {
            $this->addError('lines', __('Le montant de la proforma doit être supérieur à 0.'));
            $this->dispatch('toast', type: 'warning', title: __('Le montant de la proforma doit être supérieur à 0.'));
        ...
        if ($this->proforma) {
            $this->redirect(route('pme.proformas.show', $this->proforma).'?send=1', navigate: true);
        }
    }
    ...
    public function previewPdf(): void
    {
        ...
        Cache::put(
            ProposalDocumentPreviewController::PROFORMA_CACHE_PREFIX.$tempId,
            [
        ...
        $this->dispatch('open-pdf', url: route('pme.proformas.preview', ['tempId' => $tempId]));
    }
    ...
    private function validateForm(): void
    {
        $this->validate([
            ...
            'lines.required'               => __('Ajoutez au moins une ligne à votre proforma.'),
            'lines.min'                    => __('Ajoutez au moins une ligne à votre proforma.'),
    ...
    private function buildData(): array
    {
        return [
            ...
            'dossier_reference' => $this->emptyToNull($this->dossierReference),
            'payment_terms'     => $this->emptyToNull($this->paymentTerms),
            'delivery_terms'    => $this->emptyToNull($this->deliveryTerms),
...
<div class="flex h-full w-full flex-1 flex-col gap-6 pb-24 lg:pb-6"
    ...
    <section class="app-shell-panel overflow-hidden">
            ...
            <div>
                <div class="flex items-center gap-3">
                    <h2 class="text-2xl font-semibold tracking-tight text-ink">
                        {{ $isEditing ? __('Modifier la proforma') : __('Nouvelle proforma') }}
                    </h2>
                    @if ($isEditing && $proforma)
        ...
        @if ($isEditing && $proforma?->status === ProposalDocumentStatus::Sent)
            <div class="border-t border-amber-100 bg-amber-50 px-6 py-3 text-sm text-amber-800">
                ...
                {{ __('Cette proforma a déjà été envoyée. Toute modification impactera le document.') }}
            </div>
        @endif
    </section>
    ...
    <div class="grid gap-6 xl:grid-cols-3">
        ...
        <div class="space-y-6 xl:col-span-2">
            ...
            {{-- Conditions proforma --}}
            <section class="app-shell-panel p-6">
                <h3 class="mb-1 text-sm font-semibold uppercase tracking-[0.16em] text-slate-700">{{ __('Conditions de la proforma') }}</h3>
                <p class="mb-4 text-sm text-slate-500">{{ __('Informations clés attendues par les acheteurs publics et grands comptes.') }}</p>
                <div class="grid gap-5 md:grid-cols-3">
                    <div>
                        <label class="mb-1.5 block text-sm font-medium text-slate-800">{{ __('Référence dossier / AO') }}</label>
                        <input wire:model.blur="dossierReference" type="text"
                               placeholder="{{ __('Ex : DAO N°2026/MEF/045') }}"
                               class="w-full rounded-2xl border border-slate-200 bg-slate-50/80 px-4 py-3 text-sm text-ink placeholder:text-slate-500 focus:border-primary/40 focus:outline-none focus:ring-2 focus:ring-primary/10"/>
                        @error('dossierReference') <p
                                class="mt-1 text-sm text-rose-600">{{ $message }}</p> @enderror
                    </div>
                    <div>
                        <label class="mb-1.5 block text-sm font-medium text-slate-800">{{ __('Conditions de paiement') }}</label>
                        <input wire:model.blur="paymentTerms" type="text"
                               placeholder="{{ __('Ex : 30 jours fin de mois') }}"
                               class="w-full rounded-2xl border border-slate-200 bg-slate-50/80 px-4 py-3 text-sm text-ink placeholder:text-slate-500 focus:border-primary/40 focus:outline-none focus:ring-2 focus:ring-primary/10"/>
                        @error('paymentTerms') <p
                                class="mt-1 text-sm text-rose-600">{{ $message }}</p> @enderror
                    </div>
                    <div>
                        <label class="mb-1.5 block text-sm font-medium text-slate-800">{{ __('Délai d\'exécution') }}</label>
                        <input wire:model.blur="deliveryTerms" type="text"
                               placeholder="{{ __('Ex : 15 jours ouvrés après BC') }}"
                               class="w-full rounded-2xl border border-slate-200 bg-slate-50/80 px-4 py-3 text-sm text-ink placeholder:text-slate-500 focus:border-primary/40 focus:outline-none focus:ring-2 focus:ring-primary/10"/>
                        @error('deliveryTerms') <p
                                class="mt-1 text-sm text-rose-600">{{ $message }}</p> @enderror
                    </div>
                </div>
            </section>
            ...
            {{-- Proforma lines --}}
            <x-invoicing.line-items :title="__('Lignes de la proforma')" :lines="$lines" :currency="$currency" />
            ...
            <section class="app-shell-panel p-6">
                <div class="flex items-start justify-between gap-4">
                    <div>
                        ...
                        <p class="mt-1 text-sm text-slate-500">{{ __('Ajoutez un message qui apparaîtra sur la proforma pour le client.') }}</p>
                    </div>
                    <button
                        type="button"
                ...
                @if ($hasNotes)
                    <div class="mt-5">
                        <textarea wire:model.blur="notes" rows="3"
                                  placeholder="{{ __('Ex : Mentions complémentaires…') }}"
            ...
            <div class="sticky top-6 space-y-5">
                ...
                <section class="app-shell-panel space-y-3 p-5">
                    ...
                    <button type="button" wire:click="saveAndSend"
                        ...
                        {{ $isEditing ? __('Envoyer la proforma') : __('Enregistrer et Envoyer la proforma') }}
                    </button>
                </section>
                ...
                <div class="flex items-start gap-4">
                    ...
                    <div>
                        ...
                        <p class="mt-1.5 text-sm text-slate-500">
                            {{ __('Votre proforma sera sauvegardée mais non envoyée. Vous serez redirigé vers la liste où vous pourrez retrouver votre brouillon dans l\'onglet « Brouillon ».') }}
                        </p>
                    </div>
                </div>
