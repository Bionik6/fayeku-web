<x-layouts::auth :title="__('Connexion PME')">
    <div class="flex flex-col gap-6">
        <x-auth-header :title="__('Espace PME')" :description="__('Entrez votre numéro de téléphone et votre mot de passe')" />

        <x-auth-session-status :status="session('status')" />

        <form method="POST" action="{{ route('sme.auth.login.submit') }}" class="flex flex-col gap-5">
            @csrf
            ...
            <div class="auth-field-stack">
                <div class="flex items-center justify-between gap-1">
                    ...
                    <a href="{{ route('sme.auth.forgot-password') }}" wire:navigate class="text-sm auth-link">{{ __('Mot de passe oublié ?') }}</a>
                </div>
                <input
                    name="password"
                    type="password"
        ...
        <p class="text-center text-xs leading-6 text-slate-500">
            ...
            <a href="{{ route('accountant.auth.login') }}" class="auth-link">{{ __('Connexion cabinet →') }}</a>
        </p>
    </div>
</x-layouts::auth>
