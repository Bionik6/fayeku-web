...
$classes = Flux::classes()
    ->add('flex items-center px-3 py-2 w-full focus:outline-hidden')
    ->add('rounded-lg')
    ->add('text-start text-sm font-medium')
    ->add('[&[disabled]]:opacity-50')
    ->add(match ($variant) {
        'danger' => [
            'text-slate-700 data-active:text-red-600 data-active:bg-red-50',
            '**:data-flux-menu-item-icon:text-slate-400 [&[data-active]_[data-flux-menu-item-icon]]:text-current',
        ],
        'default' => [
            'text-slate-700 data-active:bg-slate-50',
            '**:data-flux-menu-item-icon:text-slate-400 [&[data-active]_[data-flux-menu-item-icon]]:text-current',
        ]
    })
    ;
